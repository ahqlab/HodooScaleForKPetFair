package com.cmmakerclub.iot.esptouch.task;
...
import com.cmmakerclub.iot.esptouch.EsptouchResult;
import com.cmmakerclub.iot.esptouch.IEsptouchListener;
import com.cmmakerclub.iot.esptouch.IEsptouchResult;
import com.cmmakerclub.iot.esptouch.protocol.EsptouchGenerator;
import com.cmmakerclub.iot.esptouch.udp.UDPSocketClient;
import com.cmmakerclub.iot.esptouch.udp.UDPSocketServer;
import com.cmmakerclub.iot.esptouch.util.ByteUtil;
import com.cmmakerclub.iot.esptouch.util.EspNetUtil;
...
public class __EsptouchTask implements __IEsptouchTask {
    ...
    private boolean __execute(IEsptouchGenerator generator) {
        ...
        Log.d(TAG,  String.format("%s %s", gcBytes2.toString(), dcBytes2.toString()) + " (line 237): " );

        int index = 0;
        while (!mIsInterrupt) {
            long diffTime = currentTime - lastTime;
            if (diffTime >= mParameter.getTimeoutTotalCodeMillisecond()) {
                if (__IEsptouchTask.DEBUG) {
                ...
                while (!mIsInterrupt
                    ...
                    String value = String.format("SEND GUIDE: %s %d %s-%s", mParameter.getTargetHostname(),
                            mParameter.getTargetPort(),
                            String.valueOf(mParameter.getIntervalGuideCodeMillisecond()),
                            String.valueOf(mParameter.getIntervalDataCodeMillisecond())
                    );
                    Log.d(TAG, value + " (line 255): ");
                    long _millis_begin = System.currentTimeMillis();
                    long _millis_end;
                    long _millis_diff;
                    ...
                    _millis_end = System.currentTimeMillis();
                    _millis_diff = _millis_end - _millis_begin;
                    Log.d(TAG, "send gc TIME USE = " + (_millis_diff) + "  (line 269): ");
                    if (System.currentTimeMillis() - startTime > mParameter.getWaitUdpSendingMillisecond()) {
                        //Answers.getInstance().logCustom(new CustomEvent("sendData time").putCustomAttribute("ms", _millis_diff));
                        /*Answers.getInstance()
                                .logCustom(new CustomEvent("udp is send enough time")
                                .putCustomAttribute("ms", System.currentTimeMillis() - startTime));*/
                        break;
                    }
                } // end while
                lastTime = currentTime;
            } else {
                String value = String.format("SEND DATA: %s %d %s-%s", mParameter.getTargetHostname(),
                        mParameter.getTargetPort(),
                        String.valueOf(mParameter.getIntervalGuideCodeMillisecond()),
                        String.valueOf(mParameter.getIntervalDataCodeMillisecond())
                );
                Log.d(TAG, value + " (line ): ");
